...
const AD_EXCLUSIVE_KEYS = new Set([
  ...
  // 视频广告专属字段
  'video_ad', 'ad_video', 'video_creative', 'brand_logo',
  'advertiser_name', 'advertiser_id', 'brand_name',
  'video_download_url', 'ad_label', 'ad_corner_mark',
]);
...
// 广告角标/标签文字（视频广告右上角"广告"字样）
const AD_LABEL_TEXTS = ['广告', 'AD', 'Ad', '推广'];
...
function isAd(item) {
  ...
  // 6. 广告角标文字检查（视频广告右上角"广告"标签）
  const labelFields = ['label', 'tag', 'mark', 'corner_mark', 'ad_label',
                       'label_text', 'tag_text', 'show_label'];
  for (const f of labelFields) {
    if (item[f] && typeof item[f] === 'string') {
      if (AD_LABEL_TEXTS.some(l => item[f].includes(l))) return true;
    }
  }
...
try {
  ...
  // /c/f/ad/getFeedAd 和 /c/b/ad/adBid 是纯广告接口，直接返回空
  // /c/s/adNewLog 广告日志上报，已由 URL Rewrite 拦截
